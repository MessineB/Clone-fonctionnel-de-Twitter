...
<form method="post">
        ...
        <h2> Recherche d'article </h2>
            <label for="content" class="form-label">Contenu de l'article</label> 
            <input id="content" type="text" name="search-content" placeholder="Contenu du post" class="form-control form"> 
...
<?php // Si $searchs existe
 if (isset($searchs)) {
  ...
  <?php if ($search->getStatus()==0 ) { ?>
        <a href="?page=affiche&?p=1&post_id= <?=$search->getId() ?>"> Affiché le post </a>
      <?php } ?>  
      <?php if ($search->getStatus()== 1 ) {?>
        <a href="?page=modif&?p=1&post_id= <?=$search->getId() ?>"> Caché le post </a>
        <?php } ?>
       <br>
       <a href="?page=delete&?p=1&post_id= <?=$search->getId() ?>"> Delete le post </a>
      </div>
</div>
    <?php endforeach ;} 
...
if (!isset($searchs)) :
  ...
  <?php if ($post_actuel->getStatus()==0 ) { ?>
        <a href="?page=affiche&?p=1&post_id= <?=$post_actuel->getId() ?>"> Affiché le post </a>
      <?php } ?>  
      <?php if ($post_actuel->getStatus()== 1 ) {?>
        <a href="?page=modif&?p=1&post_id= <?=$post_actuel->getId() ?>"> Caché le post </a>
        <?php } ?>
       <br>
       <a href="?page=delete&?p=1&post_id= <?=$post_actuel->getId() ?>"> Delete le post </a>
      </div>
</div>
